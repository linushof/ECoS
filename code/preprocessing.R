...
s1_problems <- read_csv('data/raw/problems/study_1_problems.csv')
s2_problems <- read_csv('data/raw/problems/study_2_problems.csv')

# samples and choices

# get data sets of individual participants
s1_files <- list.files('data/raw/study_1/', pattern = '.csv', full.names = T)
s2_files <- list.files('data/raw/study_2/', pattern = '.csv', full.names = T) 

# read data sets and store as list entries  
daemons(6)
...
# remove practice trials and match IDs across studies

s1_problems <- s1_problems |> 
  filter(ID <= 120) 
...
s2_problems <- s2_problems |> 
  ...
  mutate(ID = case_when(phase=='test' ~ ID + 37 , # assign test problems the same IDs as in study 1 (81-120)
                        phase=='training1' | phase=='training2'  ~ ID + 117)) |>  # assign training problems new IDs (121-160)
  select(!c(phase:correct_FW))

# combine and prepare problem sets

problems <- bind_rows(s1_problems, s2_problems) |> 
  distinct(ID, .keep_all = T) |> # rm duplicates (high complexity problems 81-120 included in both studies)
  rename(problem = 'ID' , 
         complexity = 'CP' , 
         o1_p1 = 'p_o_1_r_1' , 
         o1_1 = 'o_1_r_1' , 
  ...
  # compute short-term winning probabilities (see helpers)
  rowwise() |> 
...
s1_dat <- s1_data |> 
  imap(\(part, i) part |>
         rename(goal = 'aim' ,
                ...
                o2 = 'O2' , 
                choice = 'selected') |>
         mutate(age = as.double(ifelse(length(unique(c(age, age.text)))==2, na.omit(unique(c(age, age.text))), NA)) ,
         ...
         select(study, 
                participant, part_short, age, gender ,
                goal, complexity, mouse , 
                phase, trial, problem, attended, o1, o2, choice) |> 
         filter(!is.na(problem)) |> # rm rows without sampling and choice data
         group_by(problem) |>
        ...
        ungroup() |> 
         # obtain session/condition information from participant ID 
        separate_wider_delim(participant, 
                             delim = '_' ,
...
s2_dat <- s2_data |>  
  imap(\(part, i) part |>
         ...
         mutate(age = as.double(ifelse(length(unique(c(age, age.text)))==2, na.omit(unique(c(age, age.text))), NA)) ,
                ...
                part_short = paste0('part',i+length(s1_dat))) |> # assign unique participant ID (no matches with study 1) 
         select(study ,
                participant, part_short, age, gender ,
                goal, complexity, mouse , 
                phase, trial, problem, attended, o1, o2, choice) |>
         filter(! ( is.na(phase) | phase == 'practice') ) |> 
         ...
         separate_wider_delim(participant, delim = '_', 
                              ...
                              too_many = 'drop', 
                              cols_remove = F) |>
         mutate(study = if_else(is.na(training), 's2', 's3') ,
                goal = if_else(as.integer(computer) %% 2 == 1, "EV", "FW"), # bug: PsychoPy did not always code the goal: all EV condition were run on odd computer numbers  
                problem = case_when(phase=='test' ~ problem + 37 , # reassign problem IDs (as above)
...
dat <- bind_rows(s1_dat, s2_dat) |> 
  mutate(gender = case_when(gender %in% c('m' , 'M', 'male', 'm\n') ~ 'm' , 
  ...
  filter(!is.na(attended)) |> # remove redundant rows that only encode final choice
  group_by(study, part_short, phase, trial) |> 
  # compute key sampling variables for each participant and trial separately
  mutate(smp = row_number() , # running (cumulative) sample size
         smp_total = max(smp) , # total sample size trial  
  ...
  group_by(study, part_short, phase, trial) |> 
  # continue: compute key sampling variables for each participant and trial separately
  ...
  rowwise() |> 
  # continue: compute key sampling variables for each participant and trial separately
  mutate(o1_srwp = rwp(o1_O = c(o1_1, o1_2, o1_3) ,  # roundwise winning probability based on sampled relative frequencies
         ...
         o2_srwp = rwp(o1_O = c(o1_1, o1_2, o1_3) , 
                       ...
                       direction = 'smaller')) |>
  ungroup() |>
  # compute short-run and long-run winner based on sampled information
...
choices <- dat |> 
  ...
  # encode whether a correct choice according to the decision goal was made (based on ground truth and sampled information)
  mutate(correct_ground = case_when(goal=='long' & choice==long ~ 1 , 
  ...
  select(study, participant, part_short, age, gender, goal, complexity, mouse, training, phase, 
         trial, problem,
